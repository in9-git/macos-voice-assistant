...
| File | Role |
|---|---|
| `launcher` (shell script) | applies volume → branches on `mode` → starts live entrypoint, or starts servers + local loop |
...
| 파일 | 역할 |
|---|---|
| `launcher`(셸 스크립트) | 볼륨 적용 → `mode` 분기 → live 진입점, 또는 서버 기동 + 로컬 루프 |
